...
public class PensionerDetailControllerTest {
	...
	void testMockMvcNotNull() {
	  ...
	  void testGetResponse() throws Exception
	  {
		  ...
		  mockMvc.perform(get("/api/PensionerDetailByAadhaar/420559429029").header("Authorization", "Bearer @token@token"))
	  ...
	  {
		  when(authorisingClient.authorizeTheRequest("Bearer @token@token")).thenReturn(true);
		  mockMvc.perform(get("/api/PensionerDetailByAadhaar/420559429020").header("Authorization", "Bearer @token@token"))
	  ...
	  {
		  //when(authorisingClient.authorizeTheRequest("Bearer @token@token")).thenReturn(true);
		  mockMvc.perform(get("/api/PensionerDetailByAadhaar/420559429029")).andExpect(status().isBadRequest());
	  }
	  ...
	  {
		  when(authorisingClient.authorizeTheRequest("Bearer @token@token")).thenReturn(true);
		  mockMvc.perform(get("/api/getAllPensioner").header("Authorization", "Bearer @token@token"))
				.andExpect(status().isOk());
	  }
	  ...
	  {
		//  when(authorisingClient.authorizeTheRequest("Bearer @token@token")).thenReturn(true);
		  mockMvc.perform(get("/api/getAllPensioner"))
